...
const Basic = ({ navigation }) => {
  ...
  const [fournis,setfournis]= React.useState([])

   React.useEffect(() => {
        axios.get("http://1af3-196-206-107-185.ngrok.io/apiPart_Fou/")
            .then((res) => {
                setfournis(res.data)
  ...
  return (
    ...
    <SafeAreaView style={{ flex: 1, padding: 10 , color: COLORS.white}}>
       <View style={styles.header}>
        <Icon name="arrow-left" size={24} onPress={()=>navigation.navigate('Entry')} />
      ...
      <ScrollView showsHorizontalScrollIndicator={false}>
          ...
          {fournis.map(po => (
        <TouchableOpacity onPress={() => { navigation.navigate('Article');dispatch(getnomfour(po.Nom_Fou)) }} style={{ marginBottom: 10, height: 140 }}>

          <View style={{ flex: 1, padding: 20, marginBottom: 10 }}>
            <View style={[
              StyleSheet.absoluteFillObject,
              { backgroundColor: '#97C6B1', borderRadius: 16 }]} />
            <Text style={styles.name}>{po.Nom_Fou}</Text>
            <Text style={styles.desc}>{po.Adr_Fou}</Text>
